...
# อนุญาตการเข้าถึงจากทุกโดเมน (ปรับตามความจำเป็น)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def handle_callback(request: Request):
    ...
    # Log สำหรับตรวจสอบ Request
    logging.info(f"Received request with signature: {signature}")
    logging.info(f"Request body (first 100 chars): {body[:100]}...")

    try:
        events = parser.parse(body.decode(), signature)
    except InvalidSignatureError as e:
        logging.error(f"⚠️ Invalid signature error: {str(e)}")
        logging.error(f"🔑 Expected secret: {channel_secret}")
        logging.error(f"📝 Received signature: {signature}")
    ...
    # Log จำนวน Events ที่ได้รับ
    logging.info(f"📨 Received {len(events)} events")

    for event in events:
        try:
            if not isinstance(event, MessageEvent):
                logging.warning(f"⚠️ Skipping non-message event: {type(event)}")
                continue

            # Log ข้อมูล Event
            logging.info(f"💌 Processing message event from {event.source.user_id if event.source else 'unknown'}")
...
- [วารสารโรคพืชและจุลชีววิทยา](https://www.tjpp.org)"""
            ...
            try:
                await line_bot_api.reply_message(
                    event.reply_token,
                    TextSendMessage(text="⚠️ เกิดข้อผิดพลาดในการประมวลผล กรุณาลองใหม่อีกครั้ง")
                )
